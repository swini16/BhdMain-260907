...
async function assertAccessibility(page, label) {
  ...
  // Third-party UI is outside the merchant theme and can be injected after
  // page load. Exclude by stable app-owned container selectors unconditionally
  // so late injection cannot race the accessibility scan.
  builder = builder
